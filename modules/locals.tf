...
//fargate
subnets_list = flatten([
  for subnet in var.ecs_fargate.ecs_service.network_configuration.subnets :
  aws_subnet.subnets["${subnet}"].id
])

security_group_list = flatten([
  for sg in var.ecs_fargate.ecs_service.network_configuration.security_groups :
  aws_security_group.security_groups["${sg}"].id
])

assign_public_ip = var.ecs_fargate.ecs_service.network_configuration.assign_public_ip
